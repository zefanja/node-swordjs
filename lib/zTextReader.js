...
function readBooksAndChapters (fd) {
    ...
    async.whilst(
        ...
        function (err) {
            //console.log("passed whilst books");
...
function getChapterBytes(inFile, inChapterNumber) {
    if (!isNaN(inChapterNumber)) {
        ...
        if (!isNaN(vkey.getVerse())) {
            blockStartPos = blockStartPos + versesForChapterPositions.verses[vkey.getVerse()-1].startPos;
            blockLen = versesForChapterPositions.verses[vkey.getVerse()-1].length;
        }

        console.log(versesForChapterPositions, vkey);
        ...
        if (exists)
            fs.stat(inFile, function(error, stats) {
                ...
                readStream.on("readable", function () {
                    zlib.inflate(readStream.read(), function (err, result) {
                        if(result) {
